      ...
      <div className="workspace">
        ...
        <section id="network" className="network-panel" aria-label={t.networkAria}>
          ...
          {graph.nodes.length ? <div className="network-stage">
            <svg ref={svgRef} className={`network-canvas ${draggingId ? 'is-dragging' : ''} ${panning ? 'is-panning' : ''}`} viewBox="0 0 900 570" role="img" aria-label={t.networkAria} onPointerMove={moveDraggedNode} onPointerUp={stopDragging} onPointerCancel={cancelInteraction} onWheel={zoomWithWheel}>
              <rect className="network-hit-area" x="0" y="0" width="900" height="570" onPointerDown={startPanning} />
              <g>
                <g className="network-edges">{graph.edges.map((edge) => { const source = positionById.get(edge.source)!; const target = positionById.get(edge.target)!; const active = selectedIds.length > 0 && (selectedIds.includes(edge.source) || selectedIds.includes(edge.target)); return <line key={`${edge.source}-${edge.target}`} x1={source.x} y1={source.y} x2={target.x} y2={target.y} className={active ? 'is-active' : ''} style={{ strokeWidth: active ? Math.min(1.65, .45 + Math.sqrt(edge.weight) * .32) : .48 }} />; })}</g>
