    ...
    <main className={`prototype-shell ${effectiveNodeShapeMode === 'circle' ? 'node-shape-circles' : ''}`}>
      ...
      {settingsOpen && <>
        ...
        <section id="settings-panel" className="settings-panel" role="dialog" aria-modal="false" aria-labelledby="settings-title">
          ...
          {mobileLite && <fieldset className="settings-section mobile-text-size-setting"><legend>{t.graphTextSize}</legend><div className="motion-intensity-control"><span>{t.graphTextSize}<output>{Math.round(graphLabelScale * 100)}%</output></span><Slider min={.5} max={3} step={.1} value={[graphLabelScale]} onValueChange={(value) => setGraphLabelScale(singleSliderValue(value))} aria-label={t.graphTextSize} /></div></fieldset>}
