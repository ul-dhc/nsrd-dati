      ...
      <div className={`workspace ${compactPanels ? 'is-compact' : ''} ${mobileLite ? 'is-mobile-lite' : ''} ${controlsPanelOpen ? '' : 'is-controls-collapsed'} ${inspectorPanelOpen ? '' : 'is-inspector-collapsed'}`}>
        ...
        <section id="network" className={`network-panel ${presentationMode ? 'is-presentation' : ''}`} aria-label={appView === 'network' ? t.networkAria : t.overviewAria}>
          ...
          {graph.nodes.length ? <div className="network-stage">
            {mobileLite && <div className="graph-scale-control mobile-graph-text-control">
              <button type="button" className="panel-visibility-toggle graph-text-size-button" onClick={() => setActiveScaleControl((current) => current === 'label' ? null : 'label')} aria-label={`${t.graphTextSize}: ${Math.round(graphLabelScale * 100)}%`} aria-expanded={activeScaleControl === 'label'} aria-controls="mobile-graph-text-size-control" title={t.graphTextSize}><Type /></button>
              {activeScaleControl === 'label' && <div id="mobile-graph-text-size-control" className="graph-scale-popover"><span>{t.graphTextSize}</span><Slider min={.5} max={3} step={.1} value={[graphLabelScale]} onValueChange={(value) => setGraphLabelScale(singleSliderValue(value))} aria-label={t.graphTextSize} /><output>{Math.round(graphLabelScale * 100)}%</output></div>}
            </div>}
